...
export default async function DashboardPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-6 py-10">
      ...
      {!profile ? (
        <p className="rounded-4xl bg-white px-4 py-8 text-center text-slate-500 shadow-[0_2px_20px_-6px_rgba(15,23,42,0.10)]">
        ...
        {tags && tags.length > 0 ? (
          <div className="mb-6 flex gap-3 overflow-x-auto pb-1">
            {tags.map((tag) => (
              <div
                key={tag.tag_id}
                className={`group relative flex w-28 shrink-0 flex-col items-center gap-2 rounded-3xl p-4 text-center transition ${
                  tag.enabled
              ...
              >
                ...
                <form action={toggleTagEnabled}>
                  <input type="hidden" name="tag_id" value={tag.tag_id} />
                  <input
                    type="hidden"
                    name="enabled"
                    value={tag.enabled ? "false" : "true"}
                  />
                  <button
                    type="submit"
                    className="rounded-full px-2 py-0.5 text-[10px] font-medium text-slate-400 hover:text-brand-700 hover:underline"
                  >
          ...
          <p className="mb-6 rounded-3xl bg-slate-50 px-4 py-6 text-center text-sm text-slate-500">
